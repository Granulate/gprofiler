...
import os
import re
# Disabled due to lack of Windows support
#import resource
# End edit
import signal
...
class PythonMetadata(ApplicationMetadata):
    ...
    def make_application_metadata(self, process: Process) -> Dict[str, Any]:
        ...
        exe_elfid = None #get_elf_id(f"/proc/{process.pid}/exe")
        libpython_elfid = None #get_mapped_dso_elf_id(process, "/libpython")
        # End edit
...
class PySpyProfiler(ProcessProfilerBase):
    ...
    def _make_command(self, pid: int, output_path: str) -> List[str]:
        cmd = [
            ...
            "raw",
            "-F",
            #"--gil", Not supported on python 3.9.13. Error: Cannot detect GIL holding in version '3.9.13' on the current platform (reason: failed to find _PyRuntime symbol)
            "--output",
            output_path,
            "-p",
            str(pid),
        ...
        logger.debug(f"making py-spy command: {cmd} ...")
        return cmd
    ...
    def _select_processes_to_profile(self) -> List[Process]:
        ...
        for process in pgrep_keyword_exe('python'):#pgrep_exe(DETECTED_PYTHON_PROCESSES_REGEX):
            try:
                ...
                logger.exception(f"Couldn't add pid {process.pid} to list. {err}")

        logger.debug(f"Filtered python processes: {filtered_procs}")
...
class PythonEbpfProfiler(ProfilerBase):
    ...
    # Disabled for lack of Windows support
    #_DUMP_SIGNAL = signal.SIGUSR2
    # End edit
    _DUMP_SIGNAL = signal.SIGFPE
