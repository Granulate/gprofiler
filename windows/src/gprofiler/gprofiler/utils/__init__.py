...
# End Edit
TEMPORARY_STORAGE_PATH = os.getenv("USERPROFILE") + f"\AppData\Local\Temp\{GPROFILER_DIRECTORY_NAME}"
...
def start_process(
    ...
    # Disabled because os.setpgrp is not supported on Windows
...
def pgrep_keyword_exe(match: str) -> List[Process]:
    logger.debug(f"preg match string: {match}")
...
def atomically_symlink(target: str, link_node: str) -> None:
    ...
    # Added to fix bug where gprofiler crashes if link_node already exists.
    if os.path.exists(link_node):
        os.remove(link_node)
    # End edit
