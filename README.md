...
As with all native programs, the Golang program must not be stripped, or additional debug info files must be provided. Otherwise, `perf` is unable to symbolicate the stacktraces of the program. In that case gProfiler will not tag the stacks as Golang and you will not see any symbols.

Make sure you are not passing `-s` to `-ldflags` (`-s` omits the symbols table; see more details [here](https://pkg.go.dev/cmd/link#hdr-Command_Line)).
