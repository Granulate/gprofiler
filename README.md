...
* `--nodejs-mode`: Controls which profiler is used for NodeJS.
     ...
     * `attachable` - Generates perf map using [node-linux-perf module](https://github.com/mmarchini-oss/node-linux-perf). This module is injected at runtime. Requires entrypoint of application to be CommonJS script. (Doesn't work for ES modules)
